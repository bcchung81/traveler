...
- 웹에서 지운 출장은 `data/.trash/`·`out/.trash/`(휴지통)에 있다. 사용자가 복구를 원하면 웹 휴지통 화면에서 복구하게 안내하고, 휴지통 폴더를 임의로 지우지 않는다.
...
## 로컬 AI 모델
- 운영 모델은 Qwen3-VL-4B-Instruct Q4_K_M(`VLM_VARIANT=4b` 기본). 8B로 되돌리려면 `VLM_VARIANT=8b ./run-app.sh restart`. 추출 캐시는 모델별 폴더라 모델을 바꾸면 새로 읽는다.

## 인터넷 없이 쓰기·최신 규정
- 인터넷이 될 때 한 번 `uv run receipt-evidence prepare`를 실행해 두면(최신 여비 규정 조회·MCP 패키지 내려받기·로컬 AI 확인) 이후에는 인터넷 없이 읽기·판정·서류 만들기가 된다.
- 법제처 인증키 `LAW_OC`는 기본값이 없다(각자 open.law.go.kr에서 무료 발급). 없으면 조회하지 않고 저장해 둔 규정으로 판정하며 "법제처 인증키가 설정되지 않아 …"가 남는다 — 사용자에게 발급·설정(README '법제처 인증키')을 안내하고, 키를 지어내거나 남의 키를 넣지 않는다.
