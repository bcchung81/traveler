...
### Task 1: 코어
- [ ] 테스트: 확인필요→지급(청구액, 한도 초과 아님), 숙박 감액→인정(한도 초과), 식비 불인정→지급(한도 초과), 감액 범위·사유 없음·판정 값 오류는 규정 유지+오류 사유, 정액 행 무시
- [ ] 테스트: 파이프라인 합계·확인필요 목록·report 내역 절·새 버전, decision 삭제 시 원래 지문, fingerprint는 manual 있을 때만 변화
- [ ] 구현 → 통과

### Task 2: 웹
- [ ] 테스트: save_decision 검증(영수증 없음·판정 값·사유·감액 범위·규정대로 삭제), save_override가 decision 보존, 판정 화면 영수증 행에만 '판정 바꾸기', 저장 후 담당자·규정 초과 칩과 합계, ?edit 열림, 숙박 상한 초과 사유 입력
- [ ] 구현 → 통과

### Task 3: 문서·검증
- [ ] SKILL 금지 규칙·README·examples/overrides.yaml
- [ ] 전체 테스트, 복사본 서버로 판정 바꾸기→서류 생성→report 확인·스크린샷, 켠 서비스 정리
